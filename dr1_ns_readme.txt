...
* Geographic location of data collection: Not Applicable (Data was digitally downloaded from existing online sources).
* Information about funding sources that supported the collection of the data: This dataset was created for the LIN7078 module assignment at Queen Mary University of London.
...
* The dataset was generated from the raw source files using an R script. The following steps were performed:
    ...
    2. Summarized: All subtitle lines from Source 2 (which had one row per line of dialogue) were grouped by the `imdb_id` column and transfromed into a single text block per movie.
    3. Combined: The metadata table (Source 1) was joined with the new summarized text table (Source 2) using the `imdb_id` column as the key.
    4. Filtered: The dataset was filtered to include only English-language films (`original_language == "en"`) with valid `runtime` data.
    5. Cleaned Genres: The `genres` column (stored as JSON text) was parsed to create a clean comma-separated list.
    ...
    7. Finalized: The `WordsPerMinute` variable was calculated by `TotalWordCount / runtime`. The text column was removed.
...
* Variable List:
	* `imdb_id` (character): Unique identifier from IMDb. Used as the primary key.
   	* `original_title` (character): The original title of the movie.
    	...
    	* `WordsPerMinute` (numeric): A measure of dialogue density (`TotalWordCount / runtime`). **What it means: The "dialogue density" or "pace" of the movie. A high number suggests a "talky" film, meanwhile a low number suggests a more visual film.
